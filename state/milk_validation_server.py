...
def _research_observability():
    ...
    if traces_path.exists():
        try:
            ...
            metrics["queries_with_research_context"] = sum(1 for t in tr_list if t.get("has_research_context"))
            metrics["queries_without_research_context"] = sum(1 for t in tr_list if not t.get("has_research_context"))
